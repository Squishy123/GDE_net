...
# LOAD IN
#dae.load_state_dict(torch.load((str(WEIGHTS_PATH) + f'/dae_training/dae_{10}_{100000}.pth')))
#dae.eval()
...
for e in range(TOTAL_EPOCHS):
    ...
    for i in range((len(MOVING_MNIST_DATASET_FLAT)//BATCH_SIZE)):
        ...
        if ep % PLT_INTERVAL == 0:
            ...
            fig1.savefig((str(RESULTS_PATH) + '/dae_training/dae_loss.png'))

        epoch_loss += predicted_loss.item()
    
        if ep % SAVE_INTERVAL == 0:
            torch.save(dae.state_dict(), (str(WEIGHTS_PATH) + f'/dae_training/dae_{e}_{ep}.pth'))
            ...
            with torch.no_grad():
                ...
                state = reg_transform(state).to(DEVICE).float().unsqueeze(0)
                
                b.imshow(dae(state).squeeze(0).squeeze(0).squeeze(0).cpu().numpy())
                b.set_title("DAE Prediction")

                fig2.savefig((str(RESULTS_PATH) + f'/dae_training/dae_reconstruction_{e}_{ep}.png'))
                plt.close(fig2)
